  ...
  return (
    <div className="container mx-auto p-4 md:p-8">
      <div className="flex justify-between items-center mb-6">
        ...
        <Link href="/dashboard/add-product" className=" rounded-2xl bg-violet-800 hover:bg-violet-400 text-white px-2 py-1 ">Add New</Link>
      </div>
      ...
      <div className="overflow-x-auto bg-base-100 shadow-xl rounded-lg border">
        <table className="table w-full ">
          ...
          <thead className="bg-base-200">
            <tr className="text-base text-gray-700">
              ...
              {/* Category: text-left দেওয়া হয়েছে যাতে বরাবর থাকে */}
              <th className="w-1/6 text-left">Category</th>
              
              {/* Price: text-left দেওয়া হয়েছে */}
              <th className="w-1/6 text-left">Price</th>
              
              {/* Actions: এটি সেন্টারেই সুন্দর লাগে */}
              ...
              <tr key={product.id} className="hover:bg-base-200/50 transition-colors">
                ...
                <td className="text-center align-middle">
                  <div className="flex justify-center gap-3">
                    <Link href={`/products/${product.id}`} className="btn btn-sm btn-circle btn-ghost tooltip" data-tip="View">
                        <FaEye className="text-blue-600 text-lg"/>
                    </Link>
                    <button onClick={() => handleDelete(product.id)} className="btn btn-sm btn-circle btn-ghost tooltip" data-tip="Delete">
        ...
        {products.length === 0 && <p className="text-center p-8 text-lg">No products found.</p>}
      </div>
    </div>
  );
}